...
int main(void)
{
    ...
    // INKPLATE10 (classic) and INKPLATE10V2 share pins/waveform (same reference
    // schematic) -- only name and has_touch differ, same precedent as INKPLATE6/6V2
    ...
    for (int i = 0; i < 2; ++i) {
        const board_config_t *cfg = ink10_variants[i];

        assert(strcmp(cfg->name, ink10_expected_names[i]) == 0);
        ...
        // Phase 1 row, cross-checked against the real Arduino waveform1[color][phase=1]
        // column (Inkplate10Driver.cpp): color0=0, color1=0, color2=0, color3=1, color4=0,
        // color5=2, color6=0, color7=0.
        ...
        assert(cfg->has_touch == ink10_expected_has_touch[i]);
