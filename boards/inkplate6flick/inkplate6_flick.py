...
class _Inkplate:
    ...
    def init(cls, i2c):
        ...
        # Frontlight
        cls.FRONTLIGHT = GpioPin(cls._PCAL6416A_1, 11, mode_output)
    ...
    @classmethod
    def wakeup_set(cls):
        cls.Wakeup = GpioPin(cls._PCAL6416A_1, 3, mode_output)
        cls.Wakeup.digital_write(1)

    @classmethod
    def wakeup_clear(cls):
        cls.Wakeup = GpioPin(cls._PCAL6416A_1, 3, mode_output)
        cls.Wakeup.digital_write(0)

    # Frontlight control
    @classmethod
    def frontlight(cls, value):
        cls.FRONTLIGHT = GpioPin(cls._PCAL6416A_1, 11, mode_output)
        cls.FRONTLIGHT.digital_write(1 if value is True else 0)

    @classmethod
    def set_frontlight(cls, value):
    ...
    def ts_init(cls, power_state):
        # This will call the Touch class ts_init method
        return Touch.ts_init(power_state)

    @classmethod
    def ts_shutdown(cls):
        # This will call the Touch class ts_shutdown method
        Touch.ts_shutdown()

    @classmethod
    def ts_get_raw_data(cls):
        # This will call the Touch class ts_get_raw_data method
        return Touch.ts_get_raw_data()

    @classmethod
    def ts_get_xy(cls, data, i):
        # This will call the Touch class ts_get_xy method
        Touch.ts_get_xy(data, i)

    @classmethod
    def ts_get_data(cls):
        # This will call the Touch class ts_get_data method
        return Touch.ts_get_data()

    @classmethod
    def ts_get_resolution(cls):
        # This will call the Touch class ts_get_resolution method
        Touch.ts_get_resolution()

    @classmethod
    def ts_set_power_state(cls, state):
        # This will call the Touch class ts_set_power_state method
        Touch.ts_set_power_state(state)

    @classmethod
    def ts_get_power_state(cls):
        # This will call the Touch class ts_get_power_state method
        return Touch.ts_get_power_state()

    @classmethod
    def ts_available(cls):
        # This will call the Touch class ts_available method
        return Touch.ts_available()

    @classmethod
    def ts_hardware_reset(cls):
        # This will call the Touch class ts_reset method
        Touch.ts_reset()

    @classmethod
    def ts_software_reset(cls):
        # This will call the Touch class ts_sw_reset method
        Touch.ts_sw_reset()

    @classmethod
    def ts_write_regs(cls, addr, buff):
        # This will call the Touch class ts_write_i2c_regs method
        return Touch.ts_write_i2c_regs(addr, buff, len(buff))

    @classmethod
    def ts_read_regs(cls, addr):
        # This will call the Touch class ts_read_i2c_regs method
        data = bytearray(4)
        if Touch.ts_read_i2c_regs(addr, data, 4):
            return data
        return bytearray(4)

    @classmethod
    def touch_in_area(cls, x, y, width, height):
        # This will call the Touch class touch_in_area method
        return Touch.touch_in_area(x, y, width, height)

    @classmethod
    def active_touch(cls):
        # This will call the Touch class ts_get_data method and return coordinates
        x_pos = [0, 0]
        y_pos = [0, 0]
        fingers = Touch.ts_get_data(x_pos, y_pos)
        if fingers > 0:
...
class Inkplate:
    ...
    def draw_image_file(self, x, y, path, invert=False):
        with open(path, "rb") as f:
            ...
            w = int((header40[7] << 24) + (header40[6] << 16) + (header40[5] << 8) + header40[4])
            h = int((header40[11] << 24) + (header40[10] << 16) + (header40[9] << 8) + header40[8])
            data_start = int((header14[11] << 8) + header14[10])

            depth = int((header40[15] << 8) + header40[14])
            total_colors = int((header40[33] << 8) + header40[32])

            row_size = 4 * ((depth * w + 31) // 32)

            if total_colors == 0:
                total_colors = 1 << depth

            palette = None

            if depth <= 8:
                palette = [0 for i in range(total_colors)]
                p = f.read(total_colors * 4)
                for i in range(total_colors):
                    palette[i] = (54 * p[i * 4] + 183 * p[i * 4 + 1] + 19 * p[i * 4 + 2]) >> 14
            # print(palette)
            f.seek(data_start)
            for j in range(h):
                # print(100 * j // h, "% complete")
                buffer = f.read(row_size)
                for i in range(w):
                    val = 0
                    if depth == 1:
                        px = int(
                    ...
                    if self.get_display_mode() == self.INKPLATE_1BIT:
                        val >>= 1

                    self.draw_pixel(x + i, y + h - j, val)

    # Touchscreen
    def touch_in_area(self, x, y, width, height):
        return _Inkplate.touch_in_area(x, y, width, height)

    def ts_init(self, pwr_state):
        return _Inkplate.ts_init(pwr_state)

    def ts_shutdown(self):
        _Inkplate.ts_shutdown()
    ...
    def active_touch(self):
        return _Inkplate.active_touch()

    # Frontlight
    def frontlight(self, value):
        _Inkplate.frontlight(value)

    def set_frontlight(self, value):
        _Inkplate.set_frontlight(value)
