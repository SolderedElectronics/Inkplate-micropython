...
class Touch:
    ...
    def tsExitBootLoaderMode(cls):
        ...
        cls.tsWriteI2CRegs(
            CYPRESS_TOUCH_BASE_ADDR, bl_command_array, len(bl_command_array)
        )
    ...
    def tsSendCommand(cls, cmd):
        try:
            cls._i2c.writeto(
                CPYRESS_TOUCH_I2C_ADDR, bytes([CYPRESS_TOUCH_BASE_ADDR, cmd])
            )
            time.sleep_ms(20)
            return True
        except OSError:
    ...
    def touchInArea(cls, x1, y1, w, h):
        ...
        # If no new touch, check if we have a recent touch that's still valid
            ...
            if (
                cls.touchN == 1
                and BOUND(x1, cls.touchX[0], x2)
                and BOUND(y1, cls.touchY[0], y2)
            ):
                return True
            if cls.touchN == 2 and (
                (BOUND(x1, cls.touchX[0], x2) and BOUND(y1, cls.touchY[0], y2))
                or (BOUND(x1, cls.touchX[1], x2) and BOUND(y1, cls.touchY[1], y2))
            ):
                return True
